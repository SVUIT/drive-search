...
async function performSearch() {
  ...
  try {
    if (type === 'subjects') {
      ...
      const url = query ? `/search?query=${encodeURIComponent(query)}` : `/subjects`;
...
function getSelectedTags() {
  ...
  return Array.from(tagsCheckboxes).filter(c => c.checked).map(c => c.value);
}
...
function updateSelectedTags() {
  ...
  const selectedTags = Array.from(document.querySelectorAll('#tags-container input[type="checkbox"]:checked')).map(c => c.value);
  if (tagsSelected) {
    if (selectedTags.length === 0) tagsSelected.textContent = 'Chọn tags';
    else if (selectedTags.length === 1) tagsSelected.textContent = selectedTags[0];
    else tagsSelected.textContent = `${selectedTags.length} đã chọn`;
  }
}
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ${documents.map(doc => `
            <tr>
              ...
              <td class="px-6 py-4 whitespace-nowrap"><span class="px-2 py-1 text-xs rounded-full ${getTypeBadgeClass(doc.type)}">${doc.type}</span></td>
...
async function renderDocumentSearchResults(documents) {
  ...
  if (docContainer) docContainer.innerHTML = createDocumentTable(documents);
}
